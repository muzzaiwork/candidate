# [에잇퍼센트] Celery를 이용한 대규모 지급(이체) 비동기 처리
...
- **자동 재시도 (Retry Mechanism)**: 일시적인 네트워크 오류 시 Celery의 `retry` 기능을 활용하여 지수 백오프(Exponential Backoff) 방식으로 재시도 수행.
    - **지수 백오프(Exponential Backoff)란?**: 재시도 횟수가 늘어날 때마다 대기 시간을 지수적으로 늘려가는 방식입니다. (예: 1초, 2초, 4초, 8초...)
    - **도입 이유**: 장애 발생 시 즉시 반복 재시도를 하면 대상 시스템(뱅킹 API)의 부하를 가중시키고 장애 복구를 늦출 수 있으므로, 재시도 간격을 넓혀 시스템의 자정 시간을 확보하기 위함입니다.
