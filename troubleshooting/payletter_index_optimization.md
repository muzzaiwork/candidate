...
#### 2. 인덱스 파편화 (Secondary/Potential Factor)
- **오해와 진실**: 인덱스 생성 직후 성능이 떨어진 것은 **파편화 때문이 아닙니다.** 앞서 언급한 '잘못된 실행 계획'과 'Key Lookup'이 99%의 원인입니다.
- **파편화의 역할**: 파편화는 인덱스가 운영되면서 서서히 발생하는 **후행적 요인**입니다. 실행 계획이 이미 'Key Lookup'으로 잡혀 비효율적인 상황에서, 인덱스 페이지마저 물리적으로 흩어져 있다면(파편화) 성능 저하가 더욱 가속화될 뿐입니다.
- **결론**: 인덱스를 새로 만들자마자 느려졌다면, 파편화를 의심하기보다 **실행 계획이 어떻게 바뀌었는지(Execution Plan)**를 먼저 확인해야 합니다.
...
#### 6. 신규 생성 후 즉시 Rebuild가 필요할까? (FAQ)
- **결론**: **아니오, 일반적으로 필요하지 않습니다.**
- **이유**: 인덱스를 `CREATE` 하는 과정 자체가 데이터를 정렬하여 리프 페이지를 순차적으로 채우는 과정입니다. 즉, 생성 직후의 인덱스는 파편화가 거의 없는 최적의 상태입니다.
- **예외 상황**: 인덱스 생성 직후에 대량의 데이터를 `INSERT` 하거나 `UPDATE` 하여 페이지 분할이 대거 발생했다면 Rebuild가 의미가 있을 수 있지만, 단순 생성 직후에 다시 Rebuild를 하는 것은 자원 낭비입니다.
