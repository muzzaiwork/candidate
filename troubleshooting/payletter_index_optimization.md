...
#### 1. 신규 비클러스터형 인덱스의 파편화 (Index Fragmentation)
- **상관관계**: "단순히 인덱스를 추가했을 뿐인데 왜 파편화가 문제인가?"에 대한 해답입니다.
    - `reg_datetime` 컬럼에 인덱스를 생성하면, DB는 이 시간 순서대로 정렬된 **새로운 인덱스 전용 공간(페이지들)**을 만듭니다.
    - 데이터가 1초에 수백 건씩 마구 들어오거나, 과거 데이터가 수정/삭제되면 이 **인덱스 페이지들** 사이사이에 새로운 데이터를 끼워 넣어야 합니다.
