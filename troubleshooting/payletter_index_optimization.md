...
인덱스 생성 직후, `reg_datetime` 조건이 전혀 없는 조인 쿼리들의 성능이 갑자기 급격히 저하되었습니다. 쿼리문은 동일했으나, DB의 **Optimizer(CBO)**가 실행 계획을 변경하면서 시스템 전반의 응답 속도가 느려지는 장애가 발생했습니다.

```sql
-- [장애가 발생한 조인 쿼리 예시]
-- reg_datetime 조건이 전혀 없음에도 성능이 급격히 저하됨
SELECT c.*, u.username
FROM cash c
...
`cash` 테이블에 마땅한 인덱스가 없어, 대량의 데이터를 효율적으로 처리하기 위해 **Hash Join**을 사용하던 안정적인 상태입니다.
```mermaid
graph TD
    subgraph "Hash Join (안정적인 일괄 처리)"
        U["users (Build Input)"] --> H["Hash Table 생성"]
        C["cash (Probe Input)"] --> S["Clustered Index Scan"]
        S --> H
        H --> R["결과 반환"]
...
신규 인덱스(`reg_datetime`)가 추가되자, Optimizer는 이 인덱스를 타는 것이 '더 가볍다'고 오판하여 **Nested Loop Join**으로 전략을 변경합니다. 이것이 재앙의 시작이었습니다.
```mermaid
graph TD
    subgraph "Nested Loop Join (재앙의 시작: 반복의 함정)"
        U_Outer["users (Outer 테이블)"] -- "1. ACTIVE 필터 후 Loop" --> C_Inner["cash (Inner 테이블)"]
        
        C_Inner -- "2. 더 얇은 인덱스 스캔 선택" --> N["IX_CASH_REGDT Scan"]
        N -- "3. SELECT c.* 추출을 위해" --> K["Key Lookup 폭발 (Random I/O)"]
        K --> C_L["Clustered Index 접근 (cashno)"]
        
        note2["<b>문제의 핵심: 무의미한 반복</b><br/>조인 결과가 50만 건이라면<br/>IX_CASH_REGDT 탐색 50만 번 +<br/>Key Lookup 50만 번이 강제됨"]
    end
...
#### 🧐 왜 Optimizer는 `reg_datetime` 인덱스를 선택했는가?

1.  **Non-Clustered Index는 "더 가볍다"는 착시**:
    *   MSSQL Optimizer는 **Cost 기반**으로 플랜을 선택합니다.
    *   `IX_CASH_REGDT`는 `reg_datetime`과 `cashno`(PK)만 포함하므로, 모든 컬럼을 가진 Clustered Index보다 **페이지 수가 훨씬 적고 가볍습니다.**
    *   Optimizer 입장에서는 "어차피 `cash` 전체를 읽어야 한다면, 더 얇은 인덱스를 먼저 읽는 게 싸다"라고 판단하여 `reg_datetime` 조건이 없어도 이를 선택한 것입니다.

2.  **통계(Statistics) 생성으로 인한 조인 전략 변경**:
    *   인덱스 생성 시 `reg_datetime` 컬럼에 대한 **새 통계가 생성**되면서 Optimizer가 카디널리티(데이터 건수)를 재계산했습니다.
    *   이 과정에서 기존의 Hash Join보다 **Nested Loop + 신규 인덱스 스캔** 전략이 비용이 더 낮다고 잘못 추정되었습니다.
